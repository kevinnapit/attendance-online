<!-- home.php -->
<?php $this->extend('front/layout_profile/main') ?>
        ...
        <div class="card mb-3 btn-reveal-trigger">
            <div class="card-header position-relative min-vh-25 mb-8">
                ...
                <div class="avatar avatar-5xl avatar-profile shadow-sm img-thumbnail rounded-circle">
                    <div class="h-100 w-100 rounded-circle overflow-hidden position-relative"> <img src="<?php if (session()->get('user_picture')) {
                                                                                                                echo base_url() . getenv('dir.upload.profile') . session()->get('user_picture') ?><?php } else {
                                                                                                                                                                                                    echo base_url() ?>assets/img/team/avatar.png<?php } ?>" width="200" alt="" data-dz-thumbnail id="profile-image-preview">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <input class="form-control" id="name" name="name" type="text" value="<?= session()->get('user_name') ?>" placeholder="Name">
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label for="username">Username</label>
                                <input class="form-control" id="username" name="username" type="text" value="<?= session()->get('user_username') ?>" placeholder="Username">
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label for="email1">Email</label>
                                <input class="form-control" id="email1" name="email" type="text" value="<?= session()->get('user_email') ?>" placeholder="example@domain.com">
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        $('#profile-image').on('change', function(e) {
            ...
            $.ajax({
                url: "<?= site_url('user/profile/change_picture') ?>",
                type: 'POST',
                headers: {
        ...
        $('#change_password').on('submit', function(e) {
            ...
            $.ajax({
                url: "<?= site_url('user/profile/change_password') ?>",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        $('#change_profile').on('submit', function(e) {
            ...
            var formData = new FormData(form);
            $.ajax({
                url: "<?= site_url('user/profile/change_profile') ?>",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
