...
class Login extends BaseController
{
    ...
    {
        if (get_cookie('user_cookie_username') && get_cookie('user_cookie_password') && (get_cookie('user_cookie_password') != '1')) {
            $username = get_cookie('user_cookie_username');
            $password = get_cookie('user_cookie_password');
            $dataAkun = $this->model->getData($username);
            if (!password_verify($password, $dataAkun['password'])) {
                set_cookie("user_cookie_failed", '1', 3600);
            ...
            $akun = [
                ...
                'user_name' => $dataAkun['name'],
                'user_email' => $dataAkun['email'],
                'user_role' => $dataAkun['role'],
                'user_id' => $dataAkun['id'],
            ];
            session()->set($akun);
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($remember_me == '1') {
                set_cookie("user_cookie_username", $username, 3600 * 24 * 30);
                set_cookie("user_cookie_password", $password, 3600 * 24 * 30);
            }
